...
Bayesian methods have been applied in previous studies to hydrodynamic simulations initialized with a two-component Monte Carlo Glauber (MC-Glb.) model and initial conditions derived from Color Glass Condensate effective field theory using the Monte Carlo Kharzeev-Levin-Nardi (MC-KLN) model. One approach to improving current constraints on the QGP transport coefficients is to extend this coverage to more modern calculations of the QGP initial conditions and evaluate each model's performance using an appropriately chosen measure that quantifies the agreement of each model with experiment.

A second approach, which was introduced in a comprehensive Bayesian analysis of heavy-ion collisions at RHIC and the LHC [], is to replace derivative theory calculations with flexible parameterizations of the initial conditions that can mimic the full range of reasonable models. The approach effectively absorbs systematic uncertainties associated with different theory calculations into underconstrained parameters of the Bayesian analysis. 


In this work we utilize a complementary ansatz to the two-component parameterization which mimics the behavior of CGC calculations and extends the coverage of existing parametric mappings to span the full space of reasonable initial condition models.
