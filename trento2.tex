...
Another CGC model which has attracted recent interest after it successfully described an extensive list of experimental particle multiplicity and flow observables is the EKRT model which combines collinearly factorized pQCD minijet production with a simple conjecture for gluon saturation. The energy density predicted by the EKRT model after a pre-thermal stage described by Bjorken free streaming is given by
\begin{equation}
    e(\tau_0, x, y) \sim \frac{K_\text{sat}}{\pi} p_\text{sat}^3(T_A, T_B).
\end{equation}
The energy density can then be recast as an entropy density using the power law relation ${s \sim e^{3/4}}$ of the QCD EoS at high temperature.

We note that Eq.~\eqref{ekrt2} is expressed as a function of nuclear thickness $T$ which is different than the participant density $\T$ used previously. The nuclear thickness functions are typically constructed from Gaussian nucleons with infinite tails and hence result in ill defined cross sections when passed through an analytic mapping such as Eq.~\eqref{ekrt2}. In the EKRT model this issue is resolved by imposing a minimum nucleon-nucleon collision criteria $r_\text{min} = \sqrt{\sigma_\text{NN}/\pi}$ which restores Glauber cross sections.

On a practical level, differences between the use of $T$ and $\T$ are typically small when the entropy deposition mapping vanishes for zero projectile or target density--as is the case with the EKRT model--since the mapping naturally removes contributions from nucleons which are non-overlapping. We thus redefine $T_A$ and $T_B$ in Eq.~\eqref{ekrt2} as $\T_A$ and $\T_B$ in order to permit direct comparison with the wounded nucleon, KLN and \trento\ models. The approximation has also been checked by expressing the participant density as a function of nuclear thickness using a standard optical Glauber mapping to express all mappings as a function of $T_A$ and $T_B$.


%and IP-Glasma which combines the IP-Sat (impact parameter dependent saturation) model with classical Yang-Mills dynamics to describe the pre-equilibrium evolution of produced glasma fields.
