...
A primary goal of the ongoing effort is to determine the temperature dependence of QGP transport coefficients such as the specific shear viscosity $\eta/s$, theorized to reach a lower bound ${\eta/s \ge 1/4\pi}$ near the QGP phase transition temperature \cite{Policastro:2001yc, Kovtun:2004de}.
...
Bayesian methods have been applied to heavy-ion collisions in several previous studies \cite{Novak:2013bqa, Pratt:2015zsa, Sangaline:2015isa, Bernhard:2015hxa}, including simulations initialized with a two-component Monte Carlo Glauber (MC-Glb.) model \cite{Miller:2007ri} and the Kharzeev-Levin-Nardi (MC-KLN) model \cite{Drescher:2006pi}, an implementation of Color-Glass Condensate (CGC) effective field theory \cite{Gelis:2010nm, Gelis:2015gza}.
...
We use VISH2+1 \cite{Song:2007ux}, a stable, extensively tested implementation of boost-invariant viscous hydrodynamics which has been updated to handle fluctuating event-by-event initial conditions \cite{Shen:2014vra} and incorporate bulk viscous corrections with shear-bulk coupling \cite{}.
...
The parameter $(\eta/s)_\text{hrg}$ therefore only influences the small fraction of hydrodynamic evolution both below $T_c$ and before switching to the kinetic model, and is thus not expected to strongly affect the overall model.
...
We use the common shear correction \cite{Teaney:2003kp}
\begin{equation}
...
UrQMD implements a Monte Carlo realization of the Boltzmann equation
\begin{equation}
  \frac{df_i(x,p)}{dt} = \mathcal{C}_i(x, p),
\end{equation}
where $f_i$ is the distribution function and $\mathcal{C}_i$ the collision kernel for particles species $i$.
The model propagates all produced hadrons along classical trajectories, samples their stochastic binary collisions, and simulates their resonance decays until they cease interacting.
...
The random weight factor $\gamma_i$ multiplying each nucleon in Eq.~\eqref{eq:participant} is added to account for minimum bias proton-proton multiplicity fluctuations and is sampled from a gamma distribution with unit mean and variance $1/k$, where $k$ is a tunable shape parameter \cite{Bozek:2013uha}.
...
The inelastic nucleon-nucleon participation probability, and hence the density of participant matter, can be calculated from a standard Glauber formalism \cite{d'Enterria:2010hd},
\begin{equation}
  P_\text{coll}(b) =
    1 - \exp\biggl[
...
\fig{thickness}{
  ...
  The gray band indicates the region bounded by the minimum and maximum values of the local participant thickness functions $\tilde{T}_A$ and $\tilde{T}_B$, while the blue band indicates the region spanned by the generalized mean of $\tilde{T}_A$ and $\tilde{T}_B$ with parameter $-1<p<1$.
...
When two heavy ions collide at fixed impact parameter $b$, their nuclear densities are shifted by a common offset $T(x\pm b,y)$ which increases the average asymmetry of local participant matter.
...
In Fig.~\ref{fig:nch_per_npart} we plot the charged particle density per participant pair at mid-rapidity as a function of participant number using model calculations from \trento\ and experimental data from PHENIX and ALICE \cite{Adare:2015bua, Aamodt:2010cz, Adam:2015ptt}.
The model curves are calculated assuming a linear relation between charged particle multiplicity and entropy $\nch \propto S$ \cite{Song:2008si}, where the proportionality constant varies with beam energy, but is constant for all collision systems at the same energy.
We choose a fixed, energy independent instantiation of the generalized mean $p=0$ which was previously shown to provide a good description of proton-proton, proton-lead and lead-lead multiplicity distributions at LHC energies \cite{Moreland:2014oya}. In principle, the parameter $p$ could possess some energy dependence, although this is not considered in Fig.~\ref{fig:nch_per_npart}.
We see that the entropy deposition mapping modeled by a generalized mean with $p=0$ provides a good description of the data at all beam energies and self consistently describes proton-lead and lead-lead multiplicities at the same collision energy.
...
Measurements of central copper-copper, gold-gold and uranium-uranium particle production at RHIC also exhibit approximate participant scaling in agreement with Eq.~\eqref{eq:homogenous} \cite{Adare:2015bua}.
...
\fig[b]{nch_per_npart}{
    Average charged particle density per participant pair $(d\nch/d\eta)/(\npart/2)$ at mid-rapidity as a function of participant number for Au+Au, Pb+Pb and p+Pb systems at various collision energies.
    Lines are \protect\trento\ calculations with generalized mean parameter $p=0$, and symbols are experimental data from PHENIX and ALICE \cite{Adare:2015bua, Aamodt:2010cz, Adam:2015ptt}.
    The average minimum bias participant number for the p+Pb system has been shifted for clarity.
}
...
\fig*{cgc_compare}{
  ...
  The EKRT mapping is shown with model parameters $K=0.64$ and $\beta=0.8$ from Ref.~\cite{Niemi:2015qia}.
...
Calculating the saturation momentum directly from the EKRT formalism is computationally intensive, hence the authors fit $p_\text{sat}$ to a parametric form listed in \cite{Niemi:2015qia} which we use here.
...
The IP-Glasma model lacks a simple analytic form for initial energy (or entropy) deposition at the QGP thermalization time, so it cannot be directly compared to the generalized mean ansatz, Eq.~\eqref{eq:genmean}.
In lieu of such a comparison, we plot the IP-Glasma and \trento\ eccentricity harmonics in Fig.~\ref{fig:ipglasma}, which provides an indirect measure of the two initial condition models.

using a geometric mean ${p=0}$ which was previously shown to reproduce the eccentricity hierarchy of $\varepsilon_2$ and $\varepsilon_3$ in IP-Glasma.
...
\fig{ipglasma}{
  ...
  IP-Glasma events \cite{Schenke:2012wb} are evaluated after $\tau=0.4$~fm/c classical Yang-Mills evolution and \protect\trento\ events after $\tau=0.4$~fm/c free streaming.
  \protect\trento\ specific parameters are $p=0$ and $k=1.4$.
  Both models use nucleon width $w=0.43$~fm and identical Woods-Saxon parameters.
...
Each \trento\ initial condition event is free streamed using the procedure in Ref.~\cite{Liu:2015nwa} for $\tau=0.4$~fm/c to mimic the weakly coupled pre-equilibrium dynamics of IP-Glasma and match the evolution time of both models.
Both calculations are performed using nucleon width $w=0.43$~fm and identical Woods-Saxon parameters.
...
The simularity suggests that \trento\ with $p \sim 0$  can effectively reproduce the scaling behavior of the IP-Glasma model, although a more detailed comparison of the two models would be necessary to establish the strength of correspondence illustrated in Fig.~\ref{fig:cgc_compare}.
