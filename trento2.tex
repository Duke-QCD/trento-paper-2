...
Current efforts to reduce uncertainties typically focus on improving theoretical descriptions of the initial conditions \cite{Schenke:2012wb, Niemi:2015qia} as well as including new sensitive observables to assess the performance of each model.
...
In addition to QGP transport coefficients, simulations depend on auxiliary inputs such as the effective nucleon width and QGP thermalization time, all of which must be simultaneously optimized.
Evaluating at a model for a single set of parameters requires thousands of individual event simulations, so direct optimization techniques quickly become intractable.

One solution to the model optimization problem is the use of modern Bayesian methods for estimating parameters of computationally intensive models \cite{OHagan:2006ba, Higdon:2008cmc, Higdon:2014tva, Wesolowski:2015fqa}.
...
Efforts to realistically model the pre-equilibrium phase of the collision include calculations in both strongly and weakly coupled limits \cite{Schenke:2012wb, Schenke:2012fw, vanderSchee:2013pia, vanderSchee:2015rta, Chesler:2015fpa}.

Efforts to understand thermalization \cite{Romatschke:2006nk, Krasnitz:2002ng, Berges:2013eia, Arnold:2004ti, Kurkela:2011ti, Heller:2012km, vanderSchee:2013pia, Rebhan:2004ur, Heller:2011ju, Janik:2006gp}.
...
\fig[b]{nch_per_npart}{
    ...
    Lines are \protect\trento\ calculations with generalized mean parameter $p=0$, and symbols are experimental data from PHENIX and ALICE \ref{?}.
...
\fig{ipglasma}{
  ...
  IP-Glasma events \ref{?} are evaluated after $\tau=0.4$~fm/c classical Yang-Mills evolution and \protect\trento\ events after $\tau=0.4$~fm/c free streaming.
