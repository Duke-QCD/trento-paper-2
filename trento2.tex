...
\begin{abstract}
  ...
  We show that initial entropy deposition is consistent with a saturation-based picture, extract a mild preference for a specific shear viscosity that increases slowly with temperature, and find a clear signal for a finite specific bulk viscosity.
\end{abstract}
...
In lieu of such a comparison, we quantify geometric properties of the IP-Glasma and \trento\ through the eccentricity harmonics $\varepsilon_n$ weighted by transverse energy density $e(x,y)$ at the QGP thermalization time:
\begin{equation}
...
Finally, we calculated the eccentricity harmonics $\varepsilon_2$ and $\varepsilon_3$ from Eq.~\eqref{eq:eccentricity}, using the $T^{00}$ component of the stress-energy tensor after the free streaming phase as the energy density.
The resulting eccentricities harmonics, pictured in Fig.~\ref{fig:ipglasma}, are in good agreement out to large impact parameter, where sub-nucleonic structure becomes important.
This similarity suggests that \trento\ with $p \sim 0$ can effectively reproduce the scaling behavior of the IP-Glasma model, although a more detailed comparison of the two models would be necessary to establish the strength of correspondence illustrated in Fig.~\ref{fig:cgc_compare}.
