...
The heavy-ion collision transport dynamics are modeled in a multi-stage approach which uses relativistic viscous hydrodynamics for the time-evolution of the QGP medium and a microscopic Boltzmann equation to simulate the dynamics of the system after hadronization. The hydrodynamic evolution is solved using an improved version of VISH2+1, a stable, extensively tested implementation of boost-invariant viscous hydrodynamics which has been updated to handle fluctuating event-by-event initial conditions. The equations of motion are described by the second-order Israel-Stewart equations in the 14-moment approximation which leads to a set of relaxation equations, 
\begin{equation}

\end{equation}
for the bulk viscous pressure $\Pi$ and shear-stress tensor $\pi^{\mu\nu}$ and a number transport coefficients which are general functions of temperature and chemical potential.

which models the strongly coupled QGP phase of the collision using relativistic viscous hydrodynamics and microscopic Boltzmann 
