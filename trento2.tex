...
these assumptions, along with the flat 10\% uncertainty, surely affect the precise result.
...
This is because hydro is in chemical equilibrium while UrQMD is not, hence $T_\text{switch}$ is the only model parameter that can influence identified particle ratios, and so when we require the model to describe identified particle yields $T_\text{switch}$ is tightly constrained.
...
As a final verification of emulator predictions and the model's accuracy, we ran a large number of events using high-probability parameters and computed observables for comparison with experiment.
...
We observe an excellent overall fit; most calculations are within 10\% of experimental data, the notable exceptions being the pion/kaon ratio and central elliptic flow, both of which are general problems within this class of models.
...
The $v_2$ mismatch in the most central bin is a manifestation of the experimental observation that elliptic and triangular flow converge to the same value in ultra-central collisions \cite{CMS:2013bza}, a phenomenon that hydrodynamic models have yet to explain \cite{Denicol:2014ywa,Shen:2015qta}.
