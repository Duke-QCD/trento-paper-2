...
\author{Jia Liu}
\affiliation{Department of Physics, Ohio State University, Columbus, OH 43210-1117}
\date{\today}
...
The normalization factor used in this work is thus equal to unity when the rescaled $(\zeta/s)(T)$ parameterization in Eq.~\eqref{zetas} equals the approximate best fit parameterization determined in Ref.~\cite{Ryu:2015vwa}.
...
The bulk viscous correction, on the other hand, is described by multiple formulations in the literature which predict significantly different behavior when either the bulk viscosity $\zeta$ or the transverse momentum $p_T$ are large \cite{Dusling:2011fd, Noronha-Hostler:2013gga}.
\textcolor{red}{We note, however, that since the bulk parameterization $(\zeta/s)(T)$ in Eq.~\eqref{zetas} is small in the allowed hydro-to-micro transition region $135 \le T_\text{sw} \le 165$~MeV used in this study, the bulk viscous corrections to the distribution function $\delta f_\text{bulk} \propto (\zeta/s)\vert_{T_\text{sw}}$ are also expected to be small and are neglected in the present work.}
...
To overcome this limitation, we employ a Bayesian method for parameter estimation with computationally expensive models \cite{OHagan:2006ba,Higdon:2008cmc,Higdon:2014tva}.
...
\subsection{Intro to Gaussian process emulators}
...
Central to the method is a statistical surrogate model that interpolates parameter space and provides fast predictions of model output at arbitrary inputs.
We use a Gaussian process emulator \cite{Rasmussen:2006gp} as a flexible, non-parametric interpolator.
...
Suppose a generic model takes a vector of $n$ input parameters $\x$ and produces a single output $y$.
If the model is evaluated at a set of $m$ points $X = \{\x_1, \ldots, \x_m\}$, it analogously produces $m$ outputs $\y = \{y_1, \ldots, y_m\}$.
Those outputs may be assumed to have a multivariate normal distribution
\begin{equation}
  \y \sim \N(\muvec, \Sigma),
\end{equation}
where
...
\end{equation}
is the $m$-dimensional mean vector from applying the mean function $\mu$ to each input, and
\begin{equation}
  \Sigma = \sigma(X, X) =
  \begin{pmatrix}
  ...
  \end{pmatrix}
\end{equation}
is the $m \times m$ covariance matrix from applying the covariance function $\sigma$ to each pair of inputs.

Now, given the training data $(X, \y)$, the predictive distribution for the model outputs $\y_*$ at another set of input points $X_*$ is
\begin{equation}
  \begin{aligned}
  ...
  \end{aligned}
\end{equation}
Thus, by assuming only that the model follows a certain covariance function $\sigma$, we obtain both the mean predicted output and corresponding uncertainty at any desired point.
...
While Gaussian processes are fundamentally scalar functions, computer models may produce multiple outputs---the present QGP evolution model certainly does.
...
Suppose a model has $p$-dimensional output, that is, given an $m \times n$ input matrix $X$, it produces $m \times p$ output $Y$.
The principal components are then computed by the linear transformation
\begin{equation}
  Z = \sqrt m \, Y U
\end{equation}
...
Often, the $p$ model outputs are strongly correlated and so a much smaller number of principal components $q~\ll~p$ account for nearly all of the model's variance.
Thus one can consider only $q$ components, thereby reducing a high-dimensional output space to a few one-dimensional problems with negligible error.


\subsection{Model parameters and observables}

Latin hypercube, postprocessing, validation


\subsection{Calibration with MCMC}

Bayes' theorem, ensemble sampling, uncertainty
...
\begin{figure*}
    \includegraphics{mode_observables_identified}
...
\begin{figure*}
    \includegraphics{mode_observables_integrated}
    \caption{Top row: model calculations of integrated yield $dN/d\eta$, identified mean $p_T$ and flow cumulants $v_n\{2\}$ as a function of collision centrality using high-likelihood parameters (listed in table \ref{table:likely_param}) determined from the posterior in Fig.~\ref{fig:posterior_identified}. Symbols with error bars show data from ALICE and lines show results of the model. Bottom row: ratio of the model divided by experiment. Gray band indicates 10\% discrepancy between the model and experiment.}
    \label{fig:mode_observables}
\end{figure*}
