...
We first generate a large number of \trento\ events using nucleon width {\color{red}$w=0.43$~fm}, entropy deposition parameter $p=0$, and fluctuation parameter $k=1.4$, which were previously shown to reproduce the ratio of ellipticity and triangularity in IP-Glasma \cite{Moreland:2014oya}.
...
Finally, we calculate the eccentricity harmonics $\varepsilon_2$ and $\varepsilon_3$ from Eq.~\eqref{eq:eccentricity}, where the energy density is obtained from the $T^{00}$ component of the stress-energy tensor after the free streaming phase.
The resulting eccentricities harmonics, pictured in Fig.~\ref{fig:ipglasma}, are in good agreement out to large values of the impact parameter $b$ where sub-nucleonic structure starts to become important.
...
\fig{ipglasma}{
  ...
  Both models use nucleon width {\color{red} $w=0.43$~fm} \cite{Schenke:2013dpa} and identical Woods-Saxon parameters.
