...
(\eta/s)(T) = 
\begin{cases}
    (\eta/s)_\text{min} + (\eta/s)_\text{slope} \cdot (T - T_c) &\text{if } T > T_c \\
    (\eta/s)_\text{hrg} &\text{if } T \le T_c
\end{cases}
\end{equation}
...
We parameterize the bulk viscosity using the  $(\zeta/s)(T)$ previously used in the literature. 
\begin{equation}
    (\zeta/s)(T) = (\zeta/s)_\text{norm} \cdot (\zeta/s)^\dagger(T) \hfill
\end{equation}

An obvious advantage of this choice is that it allows us to directly compare our results with previous extractions of $(\zeta/s)(T)$. For the 

The fluid is particlized along an isotherm $T_\text{sw}$ using the iSS particle sampler according to the Cooper-Frye prescription,
\begin{equation}
...
The shear viscous corrections to the distribution funtion are given by,
\begin{equation}
    \delta f(x^\mu, p^\mu) = f_0(x^\mu, p^\mu)(1 \pm f_0(x^\mu, p^\mu))\frac{\pi_{\mu\nu}p^\mu p^\nu}{2(e+P)\mathcal{X}\left(\frac{p u}{T})\right)}
\end{equation}
...
At this point it is important to note that the hadron resonance gas viscosity $(\eta/s)_\text{hrg}$ plays a very different role than either $(\eta/s)_\text{min}$ or $(\eta/s)_\text{max}$.
