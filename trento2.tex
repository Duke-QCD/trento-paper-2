...
We have determined initial condition and transport properties of the QGP medium produced in relativistic heavy-ion collisions using Bayesian parameter estimation methods. The primary contribution of the present work is the application of the recently developed \trento\ model to parameterize the QGP initial conditions in a flexible manner which permits the Bayesian model-to-data analysis to select high likelihood initial conditions from the data. We show that the parameterized initial conditions are highly constrained by bulk observables and that the constraints are robust to the presence of correlated model uncertainties such as the temperature dependent shear and bulk viscosities.

The QGP initial conditions are well described by entropy deposition proportional to the geometric mean of local participant nucleon density \eqref{means}. 

We find a preferred nucleon width $w\approx0.43$~fm that is in remarkable agreement with values extracted from experiment \ref{blah}.

We see very little sensitivity of the model to variations in the strength of proton-proton multiplicity fluctuations.

The data is unable to tightly constrain $(\eta/s)_\text{min}$ and $(\eta/s)_\text{slope}$ at a single beam energy but it does constrain a linear combination of the two parameters.

The simulation prefers non-zero bulk viscosity which corroborates the findings in Ref.~\ref{}.

The simulation prefers a hydro-to-micro switching temperature ${T_\text{sw} \approx 149}$~MeV which is close to the EoS pseudo-critical phase transition temperature ${T_c=154}$~MeV. Further investigation reveals that this preference is entirely controlled by chemistry.

Bayesian parameter estimation is unable to resolve the tension in the relative yield of pions, kaons and protons. 

Model predictions vary with the choice of initial conditions and hence lead to different extracted values of the QGP transport coefficients.
from model-to-data comparison using viscous relativistic fluid dynamics relies on theoretical calculations for the initial conditions of the produced medium .


The analysis shows, for the first time, that the largest source of uncertainty in hydrodynamic simulations of heavy-ion collisions can be significantly reduced once the 
